Router.configure({
	...
	waitOn: function () {
		return [
			...
			Meteor.subscribe('participantsInRace')
		];
	}
});
...
Router.map(function () {
	...
	this.route('participantsList', {path: '/deltagere'});
	...
	this.route('deltagerListe', {
		path: '/deltagere/:name',
		data: function () { return Participants.findOne({name: {$regex: new RegExp(this.params.name), $options: 'i'}}, {sort: {name: 1}}); }
	});
	this.route('racePage', {
		path: '/races/:_id',
		waitOn: function () {
